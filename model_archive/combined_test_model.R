...
# 
# # ==============================================================================
# # C. DEEP CLEANING & MASTER SEX DICTIONARY
# # ==============================================================================
# 
# # 1. Extract known sexes from Morphology Data
# morph_sex_dict <- trait_subset_temp %>%
#   mutate(raw_sex = if_else(!is.na(scriptsex) & scriptsex != "u", scriptsex, fieldsex)) %>%
#   mutate(morph_sex = case_when(
#     raw_sex %in% c("m", "pm") ~ "m", 
#     raw_sex %in% c("f", "pf") ~ "f", 
#     TRUE ~ NA_character_
#   )) %>%
#   filter(!is.na(morph_sex)) %>%
#   dplyr::select(ringnr, morph_sex) %>%
#   distinct() %>%
#   # If there is a rare field error where a bird is "m" and "f", just take the first
#   group_by(ringnr) %>% slice(1) %>% ungroup() 
# 
# # 2. Extract known sexes from Fitness Data
# fit_sex_dict <- fitness_data %>%
#   mutate(
#     ringnr = sub("^.*_", "", Year_ID),
#     fit_sex = case_when(
#       tolower(Sex) %in% c("m", "pm") ~ "m",
#       tolower(Sex) %in% c("f", "pf") ~ "f",
#       TRUE ~ NA_character_
#     )
#   ) %>%
#   filter(!is.na(fit_sex)) %>%
#   dplyr::select(ringnr, fit_sex) %>%
#   distinct() %>%
#   group_by(ringnr) %>% slice(1) %>% ungroup()
# 
# # 3. Create the "Master Sex Dictionary" using coalesce 
# # (Prioritizes morph_sex, but falls back to fit_sex if morph_sex is NA)
# master_sex_dict <- full_join(morph_sex_dict, fit_sex_dict, by = "ringnr") %>%
#   mutate(final_sex = coalesce(morph_sex, fit_sex)) %>%
#   dplyr::select(ringnr, final_sex)
# 
# # 4. Map back to the main dataset and do the filtering
# trait_subset_temp <- trait_subset_temp %>%
#   left_join(master_sex_dict, by = "ringnr") %>%
#   # Assign the resolved sex back to the main 'sex' column
#   mutate(sex = final_sex) %>%
#   # NOW we filter. If it's still NA, it means we don't know the sex from ANY source.
#   filter(!is.na(sex)) %>%
#   
#   # Continue with Age formatting exactly as before
#   mutate(
...
# 
# cat("Birds recovered by Master Sex Dictionary check! New N: ", nrow(trait_subset_temp), "\n")
# 
...
# traits_to_check <- c("ving_h", "tars_h", "nebb_l", "nebb_h", "vekt")
# 
# investigate_df <- trait_subset_temp
# 
# 
# for(t in traits_to_check) {
#   f <- as.formula(paste(t, "~ sex + age_class"))
...
#   fit <- lm(f, data = trait_subset_temp, na.action = na.exclude)
#   
#   investigate_df[[paste0("z_", t)]] <- rstudent(fit)
#   
#   investigate_df %>%
#     filter(ringnr %in% bad_ids) %>%
#     dplyr::select(ringnr, 
...
# Create sets of unique ring numbers
ids_morph <- unique(trait_subset_lv$ringnr)
ids_repro <- unique(data_repro$ringnr)
ids_surv  <- unique(data_surv$ringnr)

# Calculate Overlaps
inventory <- data.frame(
  Dataset = c("Morphology", "Reproduction", "Survival"),
  Total_N = c(length(ids_morph), length(ids_repro), length(ids_surv)),
  Overlap_with_Morph = c(
    length(ids_morph),
    length(intersect(ids_repro, ids_morph)),
    length(intersect(ids_surv, ids_morph))
  )
)
...
#   L_init <- matrix(rnorm(dataset_joint$Nt * dataset_joint$Nlv, 0, 0.1), 
...
#     
#     # Intercepts & Slopes (Replacing Beta/X_mat)
#     alpha_trait = rnorm(dataset_joint$Nt, 0, 0.5),
#     alpha_repro = rnorm(1, -0.5, 0.5),
#     alpha_surv  = rnorm(1, 0, 0.5),
#     gamma_repro = rnorm(dataset_joint$Nlv, 0, 0.1),
#     gamma_surv  = rnorm(dataset_joint$Nlv, 0, 0.1),
#     
#     # Genetic Noise
#     w_a = matrix(rnorm(dataset_joint$Nlv * dataset_joint$Na, 0, 0.1), nrow = dataset_joint$Nlv),
#     w_pe = matrix(rnorm(dataset_joint$Nlv * dataset_joint$Na, 0, 0.1), nrow = dataset_joint$Nlv),
#     
#     # Random Effects
...
#     
#     z_year_morph = matrix(rnorm(dataset_joint$N_year * dataset_joint$Nt, 0, 0.1), nrow = dataset_joint$N_year),
#     z_year_repro = rnorm(dataset_joint$N_year, 0, 0.1),
#     z_year_surv  = rnorm(dataset_joint$N_year, 0, 0.1),
#     z_init_morph = matrix(rnorm(dataset_joint$N_init * dataset_joint$Nt, 0, 0.1), nrow = dataset_joint$N_init)
#   )
# }
...
# ==============================================================================
# out_lv <- readRDS('Output_Joint_Starter.rds')
# 
# print(out_lv,
#       pars = c("beta", "Lambda", "sd_R", "sd_year", "sd_init", "h2_lv", "rho", 
#                "h2_trait", "pe2_trait", "var_G_trait", "var_PE_trait", 
