...
X_morph <- model.matrix(~ age_class + Season_3, data = trait_subset_lv)
X_surv <- model.matrix(~ age_class, data = data_surv)
X_repro <- model.matrix(~ age_class, data = data_repro)
...
init_fn_joint <- function() {
  ...
  L_init[1, 2] <- 0.001  # Start the cross-loading practically at zero
...
saveRDS(out_joint, 'Output_everything_model.rds')
...
# 1. Convert your fit object to a 3D array [iteration, chain, parameter]
