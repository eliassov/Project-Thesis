...
fitness_data_clean <- fitness_data %>%
  ...
  mutate(
    ...
    # Extract the actual calendar year 
    Year = sub("_.*$", "", Year_ID), 
...
# Split into two clean data sets 
...
# Keep all birds that appear in any of the three data sets
...
init_fn_joint <- function() {
  ...
  L_raw_init[, 1] <- rnorm(dataset_joint$Nt, 0.7, 0.1) # Factor 1 (Size)
  ...
  list(
    ...
    a_1 = runif(1, 2.5, 3.5),
    a_2 = runif(1, 3, 4),
    delta = as.array(runif(dataset_joint$Nlv, 2, 3.5)),
    ...
    sd_R  = as.array(runif(dataset_joint$Nt, 0.5, 0.9)),
    ...
    w_a  = matrix(rnorm(dataset_joint$Na * dataset_joint$Nlv , 0, 0.1), 
                  nrow = dataset_joint$Na, ncol = dataset_joint$Nlv),
    w_pe = matrix(rnorm(dataset_joint$Na* dataset_joint$Nlv , 0, 0.1), 
                  nrow = dataset_joint$Na, ncol = dataset_joint$Nlv),
    ...
    # Scalars 
    sd_year_repro = runif(1, 0.35, 0.5),
    sd_year_surv  = runif(1, 0.25, 0.4),
